/**
 * Demote Screen - Demote user interface
 */
...
class DemoteScreen {
    ...
    async render() {
        ...
        // Draw main frame
        const { contentY } = components.drawFrame('DEMOTE USER', '[ESC] Back');
    ...
    async renderInputStage(x, y, width) {
        ...
        renderer.writeAt(x, y, `${textColor}Enter username or user ID to demote:${renderer.constructor.ANSI.RESET}`);
    ...
    async renderConfirmStage(x, y, width) {
        ...
        const confirmY = y + 10;
        renderer.writeAt(x, confirmY, `${warningColor}Demote this user?${renderer.constructor.ANSI.RESET}`);
        renderer.writeAt(x, confirmY + 2, `${textColor}This will decrease their rank by one level.${renderer.constructor.ANSI.RESET}`);
    ...
    async renderResultStage(x, y, width) {
        ...
        if (result && result.success) {
            ...
            renderer.writeAt(x, y, `${successColor}User Demoted Successfully${renderer.constructor.ANSI.RESET}`);
            ...
            renderer.writeAt(x, y, `${errorColor}Demotion Failed${renderer.constructor.ANSI.RESET}`);
    ...
    setupConfirmStageHandlers() {
        input.on('y', () => this.executeDemotion());
        input.on('n', () => {
            this.state.stage = 'input';
            this.state.error = null;
            this.render();
    ...
    async executeDemotion() {
        ...
        try {
            ...
            config.addLogEntry({
                action: 'ERROR',
                message: `Failed to demote ${this.state.user.username}: ${e.message}`
            });
        }
